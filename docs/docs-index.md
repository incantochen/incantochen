...
| `memory.md` | **durable 專案脈絡與 identity**（2026-07-22 瘦身）。品牌/客群/定位/技術選型/MVP 範圍/M-1 規劃決策脈絡——「為什麼這樣定」的敘事。**不記進度/決策清單/資料模型/里程碑**（各有專門檔）。 |
...
| 檔案 | 用途 |
|------|------|
| `tasks-todo.csv` | **待辦任務＝唯一權威**（2026-07-22 起，由舊 tasks.csv 拆出）。未開始/進行中任務，含依賴、預估、優先級、狀態；審查衍生任務也登記於此。標題帶 🚀＝上線必要（分級見 launch-scope.md）。挑任務、看依賴都讀這份。 |
| `tasks-done.md` | **已完成/取消任務的結案索引**（依階段）。每筆一行：任務·狀態·PR#·一句摘要；**完整細節看 PR/git，不重抄**。舊 tasks.csv 的長篇結案敘事已壓縮至此，原文保留於 git 歷史。 |
| `launch-scope.md` | **上線必要子集**（2026-07-08 產出）。A 硬性／B 體驗必要／C 決策擋路（含決策期限）／D 上線後可修四級分類＋外部依賴清單；「距離上線還差什麼」看這份。 |
| `decisions.csv` | **決策唯一權威（待定＋已定）**。決策項目、現況、結論、關聯任務、狀態；「這件事為什麼這樣定、在哪查」看這份（memory.md §6 舊複本已於 2026-07-22 移除）。 |
| `sprint_overview.csv` | Sprint 總覽——**2026-06-24 原始估算基準（歷史參考）**；其後新增的審查任務未計入，現況以 `tasks-todo.csv`／`tasks-done.md` 為準。 |

### 開發日誌與驗收

| 檔案 | 用途 |
|------|------|
| `work-log.md` | **接手用時間軸**（2026-07-22 瘦身）。每則幾行：動了哪些任務·產出·下一步，指向 PR#/任務、不重寫細節。跨對話接手時必讀。 |
| `work-log-archive.md` | **舊式長篇工作日誌封存**（2026-06-25 ～ 07-18）。含各任務的大表格與長敘述；歷史查閱用，非熱路徑。 |
...
| `review-findings.md` | 審查發現底稿（2026-07-22 瘦身）。dev-review 產出：審查歷程摘要、**發現一行索引**（F-###·狀態·PR#）、老化提醒、檔案覆蓋表。發現全文敘事已封存至 archive。 |
| `review-findings-archive.md` | **審查發現封存**。各 F 的完整敘事（失敗情境/修法）、審查記錄完整備註、逐日回歸狀態塊；歷史查閱用，非熱路徑。 |
...
2. `docs/tasks-todo.csv` — 當前待辦與依賴（進度唯一權威）；完成索引見 `docs/tasks-done.md`
3. `docs/work-log.md` — 最近一次作業與下一步（跨對話接手）
