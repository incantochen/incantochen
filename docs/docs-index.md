...
| `system-flow-and-user-flow.html` | **四層系統流程儀表板**（自帶樣式單檔，瀏覽器直接開）。`system-flow-and-user-flow.md` 的圖解版：四層總覽＋L1 使用者流程／L2 系統流程／L3 錯誤處理／L4 人工救援＋狀態機與對帳三臂附錄，頂部分類導覽。 |
| `order-state-machine.html` | **訂單狀態機視覺化**（自帶樣式單檔，瀏覽器直接開）。七狀態六邊的觸發者/守衛/後續動作/例外、取消守衛 TOCTOU 並發時序、對帳三臂、守衛分層總表、QA 與情境（三態複查）。`system-flow-and-user-flow.md` 附錄 A/B 的圖解版。 |
