...
| 2026-07-14 | code＋schema＋flow（跳過 env，無雲端憑證）                       | claude-opus-4-8 | F-024～F-027                                                                    | 排程審查。自上輪（2026-07-13，commit 984a0d6）後 `src/`／`supabase/` 大幅變動——合併 **PR #64（T73 三頁存取控制＋order_no 改 crypto）／#66（T88 sendOnce 回傳 boolean）／#61（T14 目錄頁）／#62（T12 選項 CRUD）／#65（T13 款式選項對應）**，migration 0014／0015 首次入審。逐行複核最高風險新程式：**T73 存取控制鏈**（`order-access-token.ts` HMAC 無狀態簽章＋伺服器端效期＋timing-safe＋`resolveOrderOwnership` 三頁一致；cookie 缺席放行保住 T111 冷連結、cookie 存在但不符才擋、限流只施加非擁有者；`generateOrderNo` 改 `randomInt` crypto）／**T88 通知鏈**（`sendOnce` 回 boolean 不破 never-throw 契約、`PAID_LINEAGE` 補寄、reconcile 加 failed-notification sweep＋per-item try/catch）／**verify-prices.ts**（162 行改寫：config_snapshot zod 驗證、白名單重算、隱藏類別／值 service-role 排除、必選完整性檢查、DB 端 base_price／price_delta 損壞防呆）／**T12／T13 選項 CRUD**（20 支 admin action 全 `requireAdmin()`、四個邏輯唯一欄位皆有 DB 約束、原子取號／交換 RPC＋樂觀鎖 CAS）／**migration 0014／0015**（is_active RLS 收斂＋排序完整性 RPC＋revoke execute，並回補 0013 漏掉的圖片 RPC revoke），**品質皆優、無 P0/P1**。新發現 F-024～F-027 全數 P2。**numeric 回傳型別疑點查證**：verify-prices 的 `typeof !== "number"` 守衛（此版之前即存在）vs notify:92「有時序列化成字串」註解看似矛盾；經 addToCart／PDP configurator 對 `numeric(12,0)` 直接算術且運作正常＋驗價守衛未擋掉真實訂單，判定運行期回傳 **number**，散落的 `Number()` 為冗餘無害、collections 內層 reduce 未包 Number 於當前運行期不致錯——**非發現**（子代理原報 collections 起價字串串接 P1 據此駁回）。T107（F-014）部分緩解（per-item try/catch 隨 T88 落地、避免整批中止）但核心順序缺口（payment 先翻 paid、無 orders 漂移補撈）仍在，見回歸狀態。 |
...
## F-024 [P2] `set_default_product_option_value`「同組至多一個 is_default」缺 DB 層 partial unique index 兜底，僅靠 RPC 為唯一寫入者維繫不變式（S7/G1）

- 狀態：待確認
- 位置：`supabase/migrations/0015_product_option_crud_support.sql:165-187`（`set_default_product_option_value()` 以 `update ... set is_default = (id = p_pov_id)` 單一 UPDATE 保證同組唯一，但 `product_option_value` 表無 `unique ... where is_default` partial index）；寫入點 `src/app/admin/products/[id]/options/actions.ts`（`addProductOptionValue` 插入 `is_default:false`、`clearDefaultProductOptionValue` 只設 false、切換走 RPC）
- 失敗情境：「每個 product_option 至多一個 is_default 值」這條不變式目前**完全靠應用層**維繫——RPC 是唯一把某值設 true 的路徑，其餘寫入點都只寫 false。只要日後任一條繞過 RPC 的寫入（新的 migration／seed 腳本／Admin 直接下 SQL／未來新增的 action 忘了走 RPC）直接寫入 `is_default=true`，同組就會出現兩個 default，而 DB 不會攔阻。後果：目錄「起」價 fallback（`collections/[category]/page.tsx:98,110` 的 `find(v => v.is_default)`）與 PDP 預設值挑選會各自取到 embed 回傳順序中的第一個 true，兩處靜默分歧、顯示不同預設價；訂單金額不受影響（verify-prices 才是權威，且它逐項重算不依賴 is_default）。屬 code-checklist **G1／schema S7（設計了防護但無 DB 約束背書，防線只存在於「目前所有寫入者剛好都守規矩」的假設上）**——與 T81（cart.member_id 設計了沒在用）、既有「約束虛設」類同源。當前無實際 exploit（RPC 確為唯一寫入者），故 P2 縱深防禦。
- 修法：加 partial unique index `create unique index uq_pov_one_default_per_option on product_option_value (product_option_id) where is_default;`（需 plan mode，屬新 migration）。與 RPC 的 `set is_default = (id = p_pov_id)` 相容（單一 UPDATE 原子切換，不會在中途出現雙 true）。加上後不變式由 DB 保證，任何繞過 RPC 的錯誤寫入會被擋成 23505 而非靜默產生雙 default。
- 記錄：2026-07-14 首次發現（本輪首次逐行審 migration 0015 與 T13 選項值預設切換路徑；對照 `uq_orders_one_pending_per_cart`／`uq_payment_one_paid_per_order` 等「不變式由 DB partial unique 背書」的既有慣例，發現 is_default 唯一性獨缺同款兜底）。

## F-025 [P2] 目錄「起」價 fallback 取未排序的 `product_option_value[0]`，與 PDP 依 `sort_order` 排序後取首值不一致：無 is_default 的選項在目錄與 PDP 顯示不同預設價

- 狀態：待確認
- 位置：`src/app/collections/[category]/page.tsx:97-103`（gem／metal 顯示 fallback `?? gemOption?.product_option_value[0]`）＋`:110`（起價加總 fallback `?? po.product_option_value[0]`）——該 embed（`:47-49`）**未帶 `order`**；對照 PDP `src/app/products/[slug]/page.tsx:61-62` 與 `src/components/product-configurator.tsx:16` 對值**先依 `option_value.sort_order` 排序**再取 `[0]`
- 失敗情境：當某 product_option **沒有任何 is_default 值**（is_default 非 DB 必填、無約束保證每組恰一個，見 F-024），目錄與 PDP 都落到「取第一個值」的 fallback。但目錄的 `product_option_value` embed 沒有 ORDER BY，`[0]` 是任意 DB 回傳序；PDP 則是排序後的首值。兩者可能取到不同的值，若這些值的 `price_delta` 不同→**目錄卡片顯示的「起」價與點進 PDP 後的預設價不一致**（例：某選項有兩個無 default 的值，價差 +0 與 +2000，目錄取到 +2000、PDP 取到 +0，客人看到卡片 NT$12,000 起、點進去卻是 NT$10,000）。屬顯示層不一致（金流不受影響，verify-prices 權威），資料良好（每組恰一 is_default）時不觸發，故 P2-low。與 F-024 相關（都因 is_default 不變式無 DB 保證而放大），但根因不同：本項是「fallback 取值兩端排序準則不一致」。
- 修法：目錄 embed 補 `option_value ( ..., sort_order )` 並在 fallback 前 `sort((a,b)=>a.sort_order-b.sort_order)` 取首值，與 PDP 對齊；或僅依賴 is_default（配合 F-024 的 partial unique 一次根治）。
- 記錄：2026-07-14 首次發現（本輪逐行審 T14 目錄頁起價算法，對照 PDP／configurator 的排序取值而暴露此不對稱；子代理平行審查亦獨立指出）。

## F-026 [P2-low] 目錄商品排序 `.order("base_price"／"created_at")` 無次要 tiebreaker：同值商品每次請求順序不定，分頁上線後將產生跳項／重複

- 狀態：待確認
- 位置：`src/app/collections/[category]/page.tsx:58-65`（`price_asc/desc` 用 `.order("base_price")`、其餘用 `.order("created_at")`，皆無次要排序鍵）
- 失敗情境：Postgres 對排序鍵相同的列不保證穩定順序。目前無分頁、單頁 ≤60 筆，影響僅「兩個同價（或同建立時間）商品的相對位置每次刷新可能不同」，客人幾乎無感。但一旦目錄成長到需要分頁／無限捲動（程式已預留 `MAX_PRODUCTS_PER_CATEGORY=60` 上限，暗示未來會分頁），無 tiebreaker 會造成分頁邊界的商品在翻頁時跳項或重複出現（同 offset 兩次查詢排序不同）。屬「為未來分頁預先設好穩定排序」的低成本防呆，故 P2-low。
- 修法：每個 `.order()` 補一個唯一次要鍵，如 `.order("base_price", {ascending}).order("id")`。一行、零風險、行為更確定。
- 記錄：2026-07-14 首次發現（本輪審 T14 目錄排序；子代理平行審查亦獨立指出）。

## F-027 [P2] `updateOptionType` 允許自由變更 `applies_to`，既有 `product_option` 映射不重新驗證：改後選項類型與掛載商品品類漂移（低信心，可能刻意）

- 狀態：待確認
- 位置：`src/app/admin/options/actions.ts:103-145`（`optionTypeUpdateSchema` 僅 omit `code`，`applies_to` 可改）；候選清單過濾在 `src/app/admin/products/[id]/options/page.tsx:53-58`（`applies_to in ('all', category)`）；`verify-prices.ts` 全程不檢查 `applies_to`
- 失敗情境：option_type X（`applies_to='ring'`）已掛在戒指商品 P 上（一列 `product_option`）。管理員把 X 編輯成 `applies_to='necklace'`。既有的 `product_option`（P↔X）不會在任何地方被重新驗證或清除——X 仍實際套用在戒指 P 上（PDP／verify-prices 照 product_option 走，不看 applies_to），但候選清單過濾器現在①不再讓 X 能重新加到別的戒指商品（若從 P 移除就加不回來），②反而讓帶著「戒指語意選項值」的 X 出現在項鍊商品的可加清單。結果是資料一致性／後台 UX 漂移：同一個選項類型同時「掛在不該掛的品類」且「無法回到原品類」。無金流影響（verify-prices 不看 applies_to、白名單仍由 product_option／product_option_value 逐項把關），故 P2。**低信心**：`option-type-codes.ts` 註解顯示團隊對 `applies_to` 漂移採「容忍、優雅降級」的立場，故此可能是刻意設計而非缺陷。
- 修法：擇一——①若視為缺陷：`updateOptionType` 在 `applies_to` 變更且該 option_type 已有 `product_option` 引用時擋下（回「此選項類型已被商品使用中，無法變更適用品類，請先移除引用」）；②若視為可接受：在 tasks／文件明記「applies_to 事後變更不回溯既有 product_option 映射」為已知行為，並在後台 UI 提示。建議由使用者做產品決策。
- 記錄：2026-07-14 首次發現（本輪逐行審 T12 `admin/options/actions.ts` 的 update 路徑，對照 T13 候選清單的 applies_to 過濾而暴露此漂移；子代理平行審查亦獨立指出並標低信心）。
...
> **2026-07-14 排程審查再確認**：自上輪（2026-07-13）後 `src/`／`supabase/` **大幅變動**——PR #64（T73）／#66（T88）／#61（T14）／#62（T12）／#65（T13）合併，migration 0014／0015 首次入審。逐條處置：**T73（F-006／F-023）已修復並複核正確**——新增 `order-access-token.ts`（無狀態 HMAC 簽章＋伺服器端效期 2h＋length-check 後 `timingSafeEqual`＋NaN 防呆），`resolveOrderOwnership` 三頁（success／pay／failed）一致套用「cookie 缺席放行保 T111 冷連結／cookie 存在但不符才擋／限流只施加非擁有者」，`generateOrderNo` 改 `randomInt`（crypto），cookie 於 createOrder 兩處簽發、admin 代客建單刻意不簽（G1 使用點齊全）→ **F-006／F-023 已標已修復**；**T88（sendOnce never-throw / 失敗信重試）已修復並複核正確**——`sendOnce` 回 boolean 不破 never-throw 契約、`ensureNotificationSent` 用 `PAID_LINEAGE` 補寄、reconcile 加 failed-notification sweep＋per-item try/catch → **既有 T88 待修項可標已修復（待使用者確認）**；**T12／T13／T14 新程式品質皆優**（見審查記錄與 F-024～F-027）。其餘逐條**確認仍在**：**T77（shipOrder write-before-validate，`admin/orders/[id]/actions.ts` 本輪未變動）／T79（findOrCreateMember 吞錯範圍待核）／T81（cart.member_id 未用）／T96（F-009：notify fallback 仍 inline slice 重組未收斂）／T99（F-012／F-021：三支 cron 已收斂 `require-cron-auth.ts` 共用 helper 但仍非 timing-safe）／T108（F-015：面交前綴仍兩處手刻）／T110（CAS＋log-insert 未交易化）**；**F-008／T95（客人端讀取忽略 `{error}`）本輪再確認仍在**——`checkout/pay/page.tsx:119` 訂單查詢、`verify-prices.ts:56/85` product／product_option 查詢、`products/[slug]/actions.ts` productOptions 查詢皆仍只解構 `data`（屬 T95／F-017 已列管群集，依去重不重報，惟 verify-prices 新增的 product_option 查詢〔:85〕與 pay 頁 orderItems 查詢〔:179〕為同根因新位置，建議併入 T95 範圍一次收斂）；**F-017／F-018／F-019／F-020／F-021／F-022 維持待確認、依去重不重報**。**T107（F-014）狀態更新**：本輪 reconcile route 因 T88 改動而重審——**核心 P1 缺口仍在**（`ecpay-reconcile/route.ts:238-249` 仍先 CAS 把 `payment.status` 翻 paid，才於 `:275-306` 呼叫 `ensureOrderPaid`；候選查詢 `:176` 仍只以 `payment.status='pending'` 為鍵、無 orders 漂移補撈）；惟 F-014 修法建議 B 的「per-item try/catch 避免單筆失敗中止整批」已隨 T88 落地（`:298`），使後果從「整批當天不對帳」縮為「該單靜默卡 pending_payment」——**部分緩解、核心未修，T107 維持列管**。T82／T83（env）本輪跳過。migration 0014／0015 首次逐行審：`is_active` 旗標＋公開讀 RLS 收斂 `using(is_active=true)`、`uq_option_value_type_sort`／`uq_product_option_product_sort` 排序完整性＋原子取號／交換 RPC（`for update` 序列化、負值 sentinel 避撞 unique、`constraint_name` 分辨排序 vs 業務衝突）、`set_default_product_option_value` 單一 UPDATE 原子切換、三支 RPC＋回補 0013 兩支圖片 RPC 全數 `revoke execute from public/anon/authenticated`，**皆合規、每個約束與 RPC 都找到程式使用點；唯一 schema 缺口＝F-024（is_default 唯一性無 partial unique 背書）**。附帶非發現：①numeric 回傳型別經查證為運行期 number（見審查記錄），子代理原報 collections 起價字串串接 P1／PDP configurator P1 據此駁回；②`env.server.ts` 的 `ORDER_ACCESS_TOKEN_SECRET` 僅驗存在未驗長度／熵，與 `CRON_SECRET`／`ECPAY_HASH_KEY` 等同一慣例，屬既有 operator-config 慣例缺口非本 delta 回歸；③`admin-notify.tsx` 的 `useAdminAction` 成功型別為 `{ok:true}`、會靜默丟棄未來可能出現的 `warning` 欄位（目前唯一回 warning 的 `shipOrder` 走 `OrderActions` 自有 handler 正確顯示，無現行受害者）——三者皆記於此供追蹤，未達發現門檻。
...
| 路徑                                                                 | 最後審查日期                     | 審查次數 |
| -------------------------------------------------------------------- | -------------------------------- | -------- |
| src/app/api/ecpay/notify/route.ts                                    | 2026-07-14                       | 5        |
| src/app/api/cron/ecpay-reconcile/route.ts                            | 2026-07-14                       | 2        |
| src/lib/ecpay/query-trade-info.ts                                    | 2026-07-07                       | 1        |
| src/lib/order/ensure-paid.ts                                         | 2026-07-14                       | 3        |
| src/lib/notification/send-once.ts                                    | 2026-07-14                       | 3        |
| src/app/api/ecpay/order-result/route.ts                              | 2026-07-07                       | 3        |
| src/app/checkout/actions.ts                                          | 2026-07-14                       | 5        |
| src/app/checkout/pay/page.tsx                                        | 2026-07-14                       | 6        |
| src/lib/quote/verify-prices.ts                                       | 2026-07-14                       | 3        |
...
| src/app/checkout/success/page.tsx                                    | 2026-07-14                       | 3        |
| src/app/checkout/success/order-status-check.tsx                      | 2026-07-07                       | 1        |
| src/app/checkout/failed/page.tsx                                     | 2026-07-14                       | 2        |
| src/app/checkout/page.tsx                                            | 2026-07-07（平行 session）       | 1        |
| src/app/cart/actions.ts                                              | 2026-07-10                       | 3        |
| src/app/cart/page.tsx                                                | 2026-07-08                       | 1        |
| src/app/products/[slug]/actions.ts                                   | 2026-07-14                       | 5        |
| src/app/products/[slug]/page.tsx                                     | 2026-07-14                       | 2        |
...
| src/lib/rate-limit.ts                                                | 2026-07-14                       | 3        |
...
| src/lib/env.server.ts                                                | 2026-07-14                       | 3        |
...
| src/app/admin/orders/checkout/actions.ts                             | 2026-07-14                       | 2        |
| src/app/admin/orders/checkout/page.tsx                               | 2026-07-13（首次，T111）         | 1        |
| src/lib/order/create-order-from-cart.ts                              | 2026-07-14                       | 2        |
...
| src/lib/storage/product-images.ts                                    | 2026-07-14                       | 2        |
...
| src/lib/order/order-access-token.ts                                  | 2026-07-14（首次，T73）          | 1        |
| src/app/checkout/rate-limited-notice.tsx                             | 2026-07-14（首次，T73）          | 1        |
| src/lib/notification/senders.ts                                      | 2026-07-14（首次，T88）          | 1        |
| src/lib/order/order-status.ts                                        | 2026-07-14                       | 2        |
| src/app/admin/options/actions.ts                                     | 2026-07-14（首次，T12）          | 1        |
| src/app/admin/options/page.tsx                                       | 2026-07-14（首次，T12）          | 1        |
| src/app/admin/options/[id]/page.tsx                                  | 2026-07-14（首次，T12）          | 1        |
| src/app/admin/options/[id]/option-type-detail.tsx                    | 2026-07-14（首次，T12）          | 1        |
| src/app/admin/options/create-option-type-form.tsx                    | 2026-07-14（首次，T12）          | 1        |
| src/lib/option/schema.ts                                             | 2026-07-14（首次，T12）          | 1        |
| src/lib/option/labels.ts                                             | 2026-07-14（首次，T12）          | 1        |
| src/app/admin/products/[id]/options/actions.ts                       | 2026-07-14（首次，T13）          | 1        |
| src/app/admin/products/[id]/options/page.tsx                         | 2026-07-14（首次，T13）          | 1        |
| src/app/admin/products/[id]/options/product-options-manager.tsx      | 2026-07-14（首次，T13）          | 1        |
| src/lib/product/product-option-schema.ts                             | 2026-07-14（首次，T13）          | 1        |
| src/lib/product/option-type-codes.ts                                 | 2026-07-14（首次，T13）          | 1        |
| src/app/collections/[category]/page.tsx                              | 2026-07-14（首次，T14）          | 1        |
| src/app/collections/[category]/loading.tsx                           | 2026-07-14（首次，T14）          | 1        |
| src/app/collections/page.tsx                                         | 2026-07-14（首次，T14）          | 1        |
| src/components/product-card.tsx                                      | 2026-07-14（首次，T14）          | 1        |
| src/components/breadcrumb.tsx                                        | 2026-07-14（首次，T14）          | 1        |
| src/components/placeholder-image.tsx                                 | 2026-07-14（首次，T14）          | 1        |
| src/components/collection-sort-select.tsx                            | 2026-07-14（首次，T14）          | 1        |
| src/lib/product/collection-sort.ts                                   | 2026-07-14（首次，T14）          | 1        |
| src/lib/product/category-labels.ts                                   | 2026-07-14（首次，T14）          | 1        |
| supabase/migrations/0014_option_crud_support.sql                     | 2026-07-14（首次，T12）          | 1        |
| supabase/migrations/0015_product_option_crud_support.sql             | 2026-07-14（首次，T13）          | 1        |

> 註（2026-07-14）：本輪逐行審 PR #64（T73 三頁存取控制）／#66（T88 sendOnce boolean）／#61（T14 目錄）／#62（T12 選項 CRUD）／#65（T13 款式選項對應）＋ migration 0014／0015 delta——首次入表 26 支業務邏輯／schema 檔（`order-access-token.ts`、`rate-limited-notice.tsx`、`senders.ts`、`admin/options/*`、`admin/products/[id]/options/*`、`option/{schema,labels}.ts`、`product/{product-option-schema,option-type-codes,collection-sort,category-labels}.ts`、`collections/*`、`product-card`／`breadcrumb`／`placeholder-image`／`collection-sort-select`、migration 0014／0015）＋複核既有金流／通知鏈（verify-prices／ecpay-reconcile／ensure-paid／send-once／notify／checkout/pay·success·failed·actions／products/[slug]·actions·page／create-order-from-cart／order-status／rate-limit／env.server／storage/product-images／admin/orders/checkout/actions，各 +1）。新程式全數 `requireAdmin()`／HMAC 存取簽章／原子 RPC／樂觀鎖 CAS 皆到位，無 P0/P1；新發現 F-024～F-027（皆 P2）。**尚未逐行審（次數＝0，留下輪輪替）**：T12／T13 展示層純 UI（`option-value-*`／`product-options-manager` 子元件的純渲染部分已隨 actions 審過主邏輯）＋上輪列出的 T09/T10/T11/T111 展示層與 `admin-*.tsx`／`image-manager`／`mobile-nav`／`flatten-field-errors`／admin/products 頁面群／loading 骨架等；`src/types/database.types.ts`（生成檔，免審）。測試檔（本輪新增 `order-access-token.test.ts`／`admin/options/__tests__/actions.test.ts`／`admin/products/[id]/options/__tests__/actions.test.ts` 等）不計入覆蓋表。numeric 回傳型別、`ORDER_ACCESS_TOKEN_SECRET` 長度、`admin-notify` warning 丟棄三項非發現記於回歸狀態 2026-07-14 段。
