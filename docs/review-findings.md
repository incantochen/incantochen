...
| 2026-07-15 | code＋schema＋flow（跳過 env，無雲端憑證）                       | claude-opus-4-8 | F-024                                                                           | 排程審查。自上輪（2026-07-13）後 `src/`／`supabase/` **大幅變動**——合併 **PR #64（T73 成功頁存取控制＋order_no 改 crypto）／#65（T13 款式選項對應）／#62（T12 選項 CRUD）／#61（T14 商品目錄頁）／#66（T88 sendOnce 回 boolean）／#68（T42 電子發票串接）／#67（T107 對帳推進順序＋T90）**，migration 0014–0016 首次入審。逐行複核最高風險新程式：**T42 電子發票鏈**（`ecpay/aes-payload`＋`invoice/{invoice-client,issue,validate,relate-number}`＋`order/{issue-invoice,invoice-meta}`＋webhook `after()` 掛載＋reconcile 未開票 sweep＋migration 0016）品質優異——AES-JSON 與 CMV 兩協議刻意分家、雙層錯誤檢查、RelateNumber 冪等＋GetIssue 判別、CAS 寫入、金額加總核對、絕不 throw 契約皆到位；**T73 修法複核正確**（HMAC 簽章 order_access_token cookie 時效＋timing-safe、cookiePresentButWrong 才擋、cookie 缺席放行保 T111 冷連結、限流只施加非擁有者→**F-006／F-023 確認已修復**）；**T107 修法複核正確**（payment CAS 移到 ensureOrderPaid 之後、單一 try/catch 包 ①②③、per-item 隔離→**F-014 確認已修復**）；**T88 複核正確**（sendOnce 回 boolean、reclaim／stale／best-effort 分支嚴謹）；**T12／T13 選項 CRUD**（migration 0014／0015 原子取號/交換 RPC＋constraint-name 分辨＋revoke execute＋RLS 收斂 is_active，前台改 `!inner` embed）與 **T14 目錄頁**皆合規、全數 admin action 有 `requireAdmin()`（D3 齊全）、無 P0/P1。新發現 **F-024（P2：T42 結帳統編／手機條碼驗證 API 在限流與購物車讀取之前呼叫→未認證放大打 ECPay＋統編轉公司名 oracle）**。既有 T77／T79／T81／T88／T95／T96／T99／T108／T110＋F-017～F-022 逐條再確認仍在（見回歸狀態）。`pnpm audit` 端點已退役無法執行（T94 已釘 postcss）。 |
| 2026-07-16 | code＋schema＋flow（跳過 env，無雲端憑證）                       | claude-opus-4-8 | 無（0 新發現）                                                                  | 排程審查。自上輪（2026-07-15，commit b913ae9）後 `src/`／`supabase/` **零變動**（HEAD 仍為 b913ae9，其間無任何 commit）——T67–T127 全數任務與 F-017～F-024 逐條維持原狀、依去重不重報。本輪價值＝**覆蓋輪替結案**：逐行補審覆蓋母集中 **39 支從未逐行審過的展示層／純 helper 檔**（老化提醒累積之欠審清單），涵蓋 admin 商品／選項 CRUD 全部 server pages＋client forms＋共用 helper（`admin-notify` 的 useAdminAction／useAdminNotify、`option/labels`、`product/collection-sort`、`option-type-codes`、`zod/flatten-field-errors`）＋前台展示元件（product-card／breadcrumb／placeholder-image／collection-sort-select／mobile-nav）＋6 支 loading 骨架＋`vitest.config`。**全數乾淨、無 P0/P1/P2 發現**。關鍵回歸確認：**F-017（Supabase `{error}` 忽略）根因未擴散到 admin 層**——所有 admin server page（`products/page`／`products/[id]/page`／`products/[id]/{images,options}/page`／`options/page`／`options/[id]/page`）皆逐一解構並檢查 `error` 後 throw（與客人端 F-017 群集相反的正面對照）、uuid 網址先驗格式避免 cast→500、全數 `requireAdmin()` 把關（D3 齊全）；client 端 swatchHex／gemColor 注入 inline style 前皆 `SWATCH_HEX_FORMAT` 格式守衛或用 React object-form（無 CSS 注入），所有 mutation 走已審的 `requireAdmin()` server action 二次驗證（client 驗證僅 UX）。至此覆蓋母集僅剩 `src/types/database.types.ts`（生成檔免審）審查次數為 0。                                                                                                                                                                                                                                                         |
...
- 狀態：✅ 已修復（2026-07-15，T107／PR #67；本輪複核正確）：`ecpay-reconcile/route.ts` 已改為①`ensureOrderPaid`（冪等 CAS）→②payment CAS 翻 paid（`.eq("status","pending")`＋`.select().maybeSingle()`）→③`ensureNotificationSent`，三步收進單一 try/catch、per-item 隔離（單筆失敗不中止整批）；①的 throw 自然跳過②③而**保留候選鍵**（payment 仍 pending），冷卻期滿隔日自動重試＝F-014 修正核心。分類掛在①的 orderResult（promoted／closed／indeterminate）而非②的 CAS 結果，漏報／計錯天的次生問題亦一併處理。webhook 端的對稱盲點（settlePaid 先推進後回應）另登記 T127（未開始）。
...
- 記錄：2026-07-15 首次發現（本輪逐行審 T42／PR #68 `checkout/actions.ts` 發票驗證接線＋`validate.ts`；對照同檔既有 `checkCheckoutGuestRateLimit` 的位置，發現新增的外部驗證呼叫落在限流與 cart 讀取之前）。
...
> **2026-07-15 排程審查再確認**：自上輪（2026-07-13）後 `src/`／`supabase/` **大幅變動**——PR #64（T73）／#65（T13）／#62（T12）／#61（T14）／#66（T88）／#68（T42）／#67（T107＋T90）合併，migration 0014–0016 首次入審。逐條處置：**T73（#15, P1）已修復並複核正確**（`order-access-token.ts` HMAC 簽章 cookie＋時效＋timing-safe；pay 頁 cookiePresentButWrong 才擋、cookie 缺席放行保 T111 冷連結、限流只施加非擁有者；`generateOrderNo` 改 `node:crypto` `randomInt`）→**F-006／F-023 改標已修復、移出待修清單**；**T107（F-014, P1→曾拉 P0）已修復並複核正確**（reconcile payment CAS 移到 `ensureOrderPaid` 之後、①②③單一 try/catch、per-item 隔離、候選鍵保留隔日重試）→**F-014 改標已修復**；**T88（sendOnce 回 boolean）複核正確**（`ensureNotificationSent` 回 boolean→webhook 回 `0|ERR` 觸發重送→reclaim 補寄＋reconcile failed sweep 兜底）；**T42 電子發票鏈品質優異、無 P0/P1**（見審查記錄，唯一新發現 F-024）；**T12／T13／T14（選項 CRUD＋目錄頁）合規**（migration 0014/0015 原子 RPC＋constraint-name 分辨＋revoke execute＋RLS is_active＋前台 `!inner`，全數 admin action `requireAdmin()`；migration 0016 `invoice_meta` jsonb 註記冪等鍵在 RelateNumber 不在本欄，合理）。**附帶確認 T14（P0）症狀消除**：`/collections/[category]` 路由已存在並正確驗證品類白名單＋檢查 `{error}`＋`!inner`。其餘逐條**確認仍在**：**T77（shipOrder write-before-validate）／T79／T81（cart.member_id 未用）／T88 never-throw 架構決策未定／T95（客人端＋account＋PDP 讀取忽略 `{error}`——本輪見 verify-prices.ts:56/85、products/[slug]/actions.ts:42/54 仍只解構 data，屬同群集未修）／T96（notify fallback `route.ts:75` 仍 inline slice 重組）／T99（三支 cron 共用 `require-cron-auth.ts` 仍裸 `!==` 非 timing-safe）／T108（面交前綴仍兩處手刻）／T110（CAS＋log-insert 未交易化）**；**F-017／F-018／F-019／F-020／F-021／F-022 逐條再確認仍在**、依去重不重報。T82／T83（env）本輪跳過。附帶觀察（非發票新發現、備查）：0014/0015 的新 RPC 為 SECURITY INVOKER＋revoke execute（僅 service role 可呼叫），未比照 0010 `create_order_with_items` 釘 `set search_path=''`；因非 SECURITY DEFINER 且執行權已收斂，風險低，僅記慣例一致性供日後評估。

> **2026-07-16 排程審查再確認**：自上輪（2026-07-15，commit b913ae9）後**無任何新 commit**（HEAD＝b913ae9），`src/`／`supabase/`／`next.config.ts`／`vercel.json` 全數零變動。故所有既有列管任務與待確認發現**逐條維持原狀、位置行號不變、依去重不重報**：**T77（shipOrder write-before-validate）／T79／T81（cart.member_id 未用）／T88 never-throw 架構決策未定／T95（客人端＋account＋PDP 讀取忽略 `{error}` 群集）／T96（notify fallback inline slice 重組）／T99（三支 cron 共用 `require-cron-auth.ts` 仍裸 `!==` 非 timing-safe）／T108（面交前綴兩處手刻）／T110（CAS＋log-insert 未交易化）／T126／T127（webhook settlePaid 對稱盲點）**逐條確認仍在；**F-017／F-018／F-019／F-020／F-021／F-022／F-024 逐條再確認仍在**、狀態維持待確認。T82／T83（env）本輪跳過（無雲端憑證）。本輪唯一實質工作為**覆蓋輪替結案**：覆蓋母集中 39 支從未逐行審過的展示層／純 helper 檔首次逐行審，全數乾淨、無新發現（詳見審查記錄與覆蓋表）。**附帶正面回歸**：T09/T10/T11/T12/T13 新增的 admin CRUD 全部 server page 對 Supabase `{error}` 的處理正確（解構＋throw、uuid 先驗、`requireAdmin()` 齊全），確認 F-017（`{error}` 忽略）根因**未擴散至 admin 讀取層**——與客人端 F-017 群集形成正面對照。flow 範圍（測試覆蓋／告警缺口／文件一致性）因程式零變動同步維持上輪結論；`pnpm audit` 端點已退役（410）無法執行，T94 已釘 postcss。
...
- **T73（#15, P1）** ✅ **已修復（2026-07-14，PR #64；2026-07-15 複核正確）**：新增 `order-access-token.ts` HMAC 簽章 cookie（時效＋timing-safe），success／pay／failed 三頁改綁擁有權（cookie 缺席放行以保 T111 冷連結、cookiePresentButWrong 才擋、限流只施加非擁有者）；`generateOrderNo` 改 `node:crypto` `randomInt`。**F-006／F-023 一併隨此修復標為已修復。**
...
- **待確認超過 14 天的發現**：目前**無任一發現逾 14 天**。距今：⚠️ **F-017（2026-07-08，距今 8 天）／F-018（2026-07-09，7 天）／F-019（2026-07-09，7 天）／F-020（2026-07-09，7 天）** 已逼近門檻，其中 F-017 達 8 天且其根因已隨 T95 群集列管未修、每輪審查重複撞見，**強烈建議使用者盡快一併裁決是否轉任務**。其餘仍待確認：**F-021（2026-07-10，6 天，P2-low）／F-022（2026-07-10，6 天，P2-low）／F-024（2026-07-15，1 天，P2）**——皆在門檻內。已處理／已修：F-006／F-023→隨 T73 已修復（PR #64）、F-014→T107 已修復（PR #67）、F-015→T108、F-002→T93、F-003→T94（已修）、F-008～F-013→T95～T100（使用者 2026-07-08 確認轉任務）、F-016 併入 T95、F-007 已修（T92）、F-005 已修（T86）、F-004 已修（T85）——均不再計入待確認。
- **從未審查過的檔案（覆蓋表中審查次數＝0）**：**本輪已清空欠審 backlog**——把上輪老化提醒列舉的 39 支從未逐行審過的展示層／純 helper 檔全數逐行補審（T42／T12/T13/T14 的 `collections/{page,[category]/loading}.tsx`／`collection-sort-select`／`product-card`／`placeholder-image`／`breadcrumb`／`mobile-nav`／`admin/options/**` 頁面群＋form＋`option-type-detail`／`admin/products/[id]/options/**` 頁面群＋`product-options-manager`／`lib/option/labels`／`lib/product/{collection-sort,option-type-codes}`；以及 T09/T10/T11/T111 遺留的 `admin-checkout-form`／`admin-product-form`／`admin-nav`／`admin-notify`／`admin-pill`／`admin-filter-pills`／`saved-banner`／`ui/skeleton`／`image-manager`／`invoice-section`／`admin/page`／`admin/products/{page,new/page,[id]/page,[id]/{images,options}/page}`／`lib/zod/flatten-field-errors`／`checkout/rate-limited-notice`＋各 `loading.tsx` 骨架＋`vitest.config`）。全數乾淨、無發現。至此覆蓋母集（`git ls-files` 排除純資產、測試檔、生成檔）中**唯一審查次數仍為 0 的檔案為 `src/types/database.types.ts`（Supabase 自動生成，免人工逐行審）**。`lib/support/{schema,support-request}.ts` 已於前輪列入覆蓋表（2026-07-10）。建議下一輪：①env 範圍（俟雲端憑證可用）；②高風險金流／schema 區隨各修復 PR 落地時複審（維持既有 delta 優先原則）。
