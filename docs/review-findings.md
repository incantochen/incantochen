...
- **T76–T79／T81（#18–#23, P2）** 訂單交易化、shipOrder 順序、cart 限流、findOrCreateMember 吞錯、cart.member_id 未用——本輪未見修復跡象。**確認仍在。**（T80 PII log 留存已於 2026-07-09／PR #48 完成。）
...
- **待確認超過 14 天的發現**：無（門檻 14 天）。仍待確認者：**F-017（2026-07-08，P2）／F-018（2026-07-09，P2-low）／F-019（2026-07-09，P2-low）／F-020（2026-07-09，本輪新發現，P2）**——皆在 14 天門檻內。已處理：F-014→T107、F-015→T108、F-002→T93、F-003→T94、F-008～F-013→T95～T100（使用者 2026-07-08 確認轉任務）、F-016 併入 T95。F-007 已轉 T92、F-005 已修（T86）、F-006 已併入 T73、F-004 已修（T85）——均不再計入待確認。
- **從未審查過的檔案（覆蓋表中審查次數＝0）**：本輪補審 `layout.tsx`／`ui/page.tsx`／`ui/button.tsx`（皆純 UI 無發現）＋新檔 `safe-redirect.ts`／migration `0008`／`0009`。至此覆蓋表僅剩 **`src/types/database.types.ts`（Supabase 自動生成檔，隨 migration 再生，無需人工審）** 審查次數＝0，其餘業務邏輯檔全數至少審過一次。建議下一輪回到高風險區複審（金流鏈 notify／checkout／`state-machine.ts` 隨各自修復 PR 落地時）與 env 範圍（俟雲端憑證可用）。
...
| src/app/admin/orders/[id]/actions.ts                                 | 2026-07-09                       | 5        |
...
| src/app/products/[slug]/actions.ts                                   | 2026-07-09                       | 3        |
