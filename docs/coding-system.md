...
- 「留給使用者本機做」的驗證項不可無限懸置：merge 後、下個任務開始前補做，或在 tasks-todo.csv 留下明確的待驗證註記。
...
- [ ] DoD：sandbox 實走驗證已完成或已在 tasks-todo.csv 留明確待驗證註記？
...
| **對 trigger 管理的欄位做 optimistic guard，拿 SET 進去的值當比對基準——但那個值從未存進 DB**（T81／PR #84 手動驗證實測抓到，單元測試全綠攔不到）：mergeCarts 的 CAS 鎖 `UPDATE cart SET updated_at=<JS toISOString>`，後續刪殼 guard `.eq("updated_at", <同一個 JS 值>)`——但 `trg_cart_updated_at`（0001 的 `set_updated_at` BEFORE UPDATE trigger）在**每次 UPDATE 都把 updated_at 無條件蓋成 DB 的 now()**，SET 值根本沒落地，guard 比對必 miss（實測：DB 值微秒尾數非零，JS 毫秒精度尾數必為 000，鐵證）→ 空殼永不刪、cookie 永不清。單元測試 mock 不會模擬 DB trigger，全綠照樣漏；靠真 DB 手動走流程（觀察 cookie 沒變）才暴露。**修法：對 trigger 管理的欄位，guard 基準一律用 UPDATE 的 RETURNING（`.select("updated_at")`）回讀 trigger 實際寫入的值，絕不假設 SET 值＝落地值**；順帶收穫：trigger 保證每次 UPDATE 必改該欄位，§3.2「SET 必須改動 WHERE 欄位」的 EvalPlanQual 前提由 trigger 免費成立 | §3.2 條件式 UPDATE 規則（比對基準要是 DB 實際值）、§5.4 mock 一致性（mock 不含 trigger 行為，真 DB 驗證不可省） |
...
| **`ADD CONSTRAINT ... NOT VALID` 與 `VALIDATE CONSTRAINT` 放進同一個 migration，宣稱的「不長鎖」線上化效益歸零**（T47／PR #86 max＋deep-domain 兩份審查交叉命中，migration 0019）：兩段式的用意是 `NOT VALID` 只取極短的 `ACCESS EXCLUSIVE`（不掃全表）先擋新寫入、`VALIDATE` 改吃較弱的 `SHARE UPDATE EXCLUSIVE`（不擋讀寫）再驗歷史列。但 Supabase CLI 逐檔在**單一交易**內執行 DDL——同交易裡 `NOT VALID` 取得的 `ACCESS EXCLUSIVE` 鎖**不會在該步結束後釋放，而是持有到交易 commit**，涵蓋整段 `VALIDATE` 掃描期間，讀寫全被阻擋，正好是兩段式要避免的行為（等同、甚至劣於直接 `ADD CONSTRAINT`）。`support_request` 表小、影響可忽略，但此 pattern 複製到 `orders`／`payment` 等大表會造成上線期間長時間鎖表。**要真正拿到兩段式好處，`VALIDATE CONSTRAINT` 必須拆到後續獨立 migration／獨立部署**（讓 `NOT VALID` 的交易先 commit 放鎖）；同一檔內寫兩段只是「看起來線上化」。註解若宣稱不長鎖，也要與實際鎖行為一致，否則誤導下一個抄這段的人 | §4.5 部署順序（DDL 鎖行為要看「交易邊界」不只單一語句）、§2.3 契約思考（註解宣稱的保證必須與實際行為相符） |
| **把「payment 翻面＋訂單狀態轉換」拆成兩次非原子寫入，中間失敗留下無自癒的金流半套狀態**（T47／PR #86 max＋deep-domain 兩份審查主根因）：記錄式退款原本先翻 `payment=refunded`、再 `transitionOrder` 轉 `orders=refunded`——第二步遇持續性錯誤或並發競態失手時，留下「payment 已退、訂單沒退」的半套狀態，而 UI（`hasPaidPayment=false` 隱藏補救入口）與 reconcile 稽核臂（只認 `orders=cancelled`）都撈不到 → 客人收不到退款信、需人工改 DB。這是 §3.1「跨表狀態不變量」＋ T110 案例（原子化時盤點交易外狀態）的**正向再犯**：明知兩表狀態要一致，卻仍分兩次寫。**兩個相關狀態必須一致時，優先包進單一交易 RPC（CAS 未命中一律 raise 令整筆 rollback，不可 return——return 會讓前半寫入隨外層交易 commit）；無法原子化的殘餘半套狀態（如 Admin Override 逃生口）必須有 durable 稽核臂以「另一半」為鍵每日撈得到＋UI 補救入口，不能只靠逐單開啟才顯示的橫幅**（漏看即永久無聲，正是稽核臂存在的理由） | §3.1 狀態機（跨表狀態不變量優先原子化）、§3.3 重試迴路（殘餘中間態要有以「另一半」為鍵的 durable 兜底） |
