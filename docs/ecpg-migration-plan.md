...
**建議時機**：**MVP 上線（M4/T38）前不做**。現行 AIO 已通過 sandbox 實測、冪等鏈完整、風險已知；ECPG 的效益（綁卡、體驗）屬回頭客優化，在有真實流量前無法變現。已登記為 **T103**（2026-07-08，tasks-todo.csv），依賴 T35（正式金鑰）、T82（環境分離）、T101（CI）完成後才動工。以下規劃按「屆時動工」撰寫。
