...
- **MVP 政策（2026-07-02 修訂）**：原定「待付款訂單不自動取消」；T65 規劃時發現待付款無時效＝客人可壓舊價很久後付款，已改列 **T66（pending_payment 逾期自動取消，如 72 小時，走狀態機轉 cancelled）**，與 T74（pending payment 逾期換新 trade no）、T75（付款完成才清購物車）同批設計。未完成提醒信仍列 Phase 2。
- **未勾選客製例外同意**：不可送出。
- **前端價≠後端價**：一律以後端重算為準並提示。
- **規格選擇依品類**：戒指走戒圍（不確定 → 量法頁）；耳環走耳針/耳夾；手鍊／項鍊走長度。
- **Email 已是會員（T71 修訂 2026-07-10）**：訪客結帳輸入的 email 命中既有會員時**不再靜默掛單**——回傳需先登入提示，導向 `/login` 完成 OTP 驗證後導回結帳頁、以已驗證身份重新送出，才綁定該會員；全新 email 訪客結帳仍自動建會員，行為不變。
- **庫存**：不擋單，以交期告知管理。
...
flowchart TD
    ...
    B --> C["系統：寄一封信<br/>含 OTP 驗證碼＋magic link（防濫發限流）"]
    C --> D{"使用者選驗證方式"}
    D -->|主推：輸入驗證碼| E["在當下裝置輸入驗證碼"]
    E --> F{"碼正確且未過期?"}
    F -->|否| R["友善錯誤・一鍵重新寄送"]
    R --> B
...
| 2   | **輸入驗證碼**（主）或**點連結→落地頁按『登入』**（輔） | 驗 OTP/token（高熵、雜湊、單次、短效）、建 session（落在當下裝置）、路由保護 | T07           |
...
- **主推 OTP 驗證碼、magic link 為輔**：驗證碼讓 session 精準落在**當下裝置**（解跨裝置：手機輸入 → 桌機收信也不卡），且 email 安全掃描器「點不掉」一個數字碼。碼長**不假設固定位數**（本機設定 6 碼、雲端 production 實際 8 碼——T06 踩坑實證，驗證邏輯採 4–10 位彈性）。
...
| 付款重試/待付款 | 可重試（速率限制＋冪等防重複扣款）・逾期自動取消已列 T66・不寄提醒                         | 未完成提醒信                |
