...
| `api/cron/ecpay-reconcile/route.ts` | 每日兜底：撈 pending 超過 10 分鐘且 24 小時內未對過帳的 payment（上限 30 筆、400ms 節流）→查 ECPay→金額吻合才 CAS 推進，否則只告警 | 「webhook＋主動對帳共用冪等鎖」紅線落地（T89）；promoted 一律 Sentry 告警供追蹤 webhook 可靠度（T88/T90） |
...
sequenceDiagram
    ...
    R->>DB: 撈 pending payment（>10 分鐘、24h 冷卻、≤30 筆）
    loop 每筆（400ms 節流）
        R->>EC: QueryTradeInfo（驗章）
        alt 已付款且金額吻合
            R->>DB: CAS UPDATE payment→paid＋ensureOrderPaid＋補寄通知
