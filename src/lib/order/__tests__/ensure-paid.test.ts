...
function makeServiceRole(opts: {
  ...
  const serviceRole: any = {
    // T110：CAS 推進＋order_status_log 寫入走 transition_order_status RPC
    // （單一交易），promote 直接餵給 rpc 的 maybeSingle。
  ...
  return { serviceRole, cartDeleteCalls, cartItemDeleteIds, rpcCall };
}
...
describe("ensureOrderPaid：T75 付款成功清購物車", () => {
  it("promoted.cart_id 非 null，cart 下單後沒被動過 → 刪除對應 cart", async () => {
    const { serviceRole, cartDeleteCalls, rpcCall } = makeServiceRole({
      promote: {
        data: {
          id: "order-1",
          cart_id: "cart-1",
    ...
    // RPC 參數契約：from/to 硬編碼、note 記錄觸發來源（ops-runbook 依賴
    // "ECPay webhook"/"ECPay reconcile" 區分推進路徑）、非 override。
    expect(rpcCall).toHaveBeenCalledWith({
      p_order_id: "order-1",
      p_from: "pending_payment",
      p_to: "paid",
      p_is_override: false,
      p_note: "ECPay webhook",
    });
  });

  it("RPC 回傳 error（DB 暫時性故障或 log 寫入失敗 rollback）→ throw、不回 promoted、不進清車流程", async () => {
    // T110 行為變更的核心：舊版 log 失敗只記 Sentry、照樣回 promoted；現在
    // log 與狀態推進同一交易，任一失敗整段 rollback、rpc 回 error——必須
    // throw 讓 webhook 回 ERR，ECPay 重送時 CAS 重新來過。
    const { serviceRole, cartDeleteCalls } = makeServiceRole({
      promote: { data: null, error: { message: "connection timeout" } },
    });

    await expect(
      ensureOrderPaid(serviceRole, "order-1", "webhook"),
    ).rejects.toThrow("ensureOrderPaid failed");
