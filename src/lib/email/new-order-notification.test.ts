...
import { sendNewOrderNotification } from "./new-order-notification";

// 收件人姓名／地址為客人自由輸入，未跳脫直接插入信件 HTML 可被注入任意
// HTML／釣魚連結，店家新訂單通知信首當其衝（T72／F-001）。驗證惡意輸入
// 不會以原始 HTML 出現在寄給店家的信件中。
...
describe("sendNewOrderNotification：HTML escape（T72／F-001）", () => {
  beforeEach(() => {
    sendMock.mockClear();
  });

  it("收件人姓名含 <script> 標籤時，寄給店家的通知信 html 不含未跳脫的原始標籤", async () => {
    mockOrderRow({
      recipientName: '<script>alert("xss")</script>',
      shippingAddress: "台北市中正區重慶南路一段1號",
    });

    await sendNewOrderNotification("order-1");
  ...
  it("地址含 HTML 屬性注入字元時同樣被跳脫", async () => {
    ...
    await sendNewOrderNotification("order-2");
